...
# ─── Branded PDF (carta, entrevista, linkedin, análisis) ──────────────────────
...
def build_branded_pdf(title: str, content_text: str, person_name: str = "") -> io.BytesIO:
    """Generic branded PDF — used for carta, entrevista, linkedin content."""
    buf = io.BytesIO()
    ...
    if os.path.exists(logo_path):
        ...
        hdr_tbl = Table([[logo, hdr_left, hdr_right]], colWidths=[2*cm, 9*cm, 6*cm])
    else:
        hdr_tbl = Table([[hdr_left, hdr_right]], colWidths=[11*cm, 6*cm])
    ...
    story.append(Paragraph(
        ...
        "analyze-this-production.up.railway.app",
        s_foot,
    ))
    doc.build(story)
    buf.seek(0)
...
def build_analysis_pdf(cv_data: dict) -> io.BytesIO:
    """PDF with the full ATS analysis: score, keywords, coaching, disclaimer."""
    ...
    lines.append(f"**Cargo analizado:** {titulo}")
    lines.append("")
    lines.append(f"**Score ATS Match: {score}%**")
    lines.append(f"ATS Compatible: {'Sí ✓' if ats_ok else 'No ✗'}")
    if ats_det:
        lines.append(f"ATS detectado: {ats_det}")
    if explain:
        lines.append("")
    ...
    if desglose:
        lines.append("")
        lines.append("**Desglose del análisis:**")
    ...
    if coaching:
        lines.append("")
        lines.append("**Tu plan de acción:**")
        for tip in coaching:
    ...
    return build_branded_pdf(
        f"Análisis ATS · {titulo or nombre}",
        "\n".join(lines),
        nombre,
    )
